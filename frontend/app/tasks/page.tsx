...
import { AlertCircle, CheckCircle2, Clock3, FileVideo, Image as ImageIcon, Loader2, Plus, RefreshCw, Sparkles } from "lucide-react";
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-[#6e6e73]">
              汇总 AI 视频、AI 图片的排队、生成、完成和失败记录，把 DA 类生成站的异步任务流集中管理。
            </p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="grid min-h-72 place-items-center p-6 text-center">
            <div>
              ...
              <p className="mt-2 text-sm text-[#86868b]">从视频或图片工作台创建一个任务后，会在这里看到队列状态。</p>
            </div>
          </div>
        ) : (
...
function TaskRow({ task }: { task: TaskItem }) {
  const Icon = task.kind === "video" ? FileVideo : ImageIcon;
  const href = task.kind === "video" ? "/video" : "/image";
      ...
      <div className="flex min-w-0 gap-3">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h2 className="truncate text-sm font-semibold">{task.kind === "video" ? "AI 视频任务" : "AI 图片任务"}</h2>
